...
extern void eph2pos(gtime_t time, const eph_t *eph, double *rs, double *dts,
                    double *var)
{
    double tk, A0,a,M, E, Ek, sinE, cosE, u, r, i, O, sin2u, cos2u, x, y, sinO, cosO, cosi, mu, omge;
    ...
    if(sys==SYS_CMP&&eph->type==1)
    {
        if ((prn >= 38 && prn <= 40) || (prn >= 59 && prn <= 63))
		{
			A0 = A_ref_igso + eph->dA;
		}
		else
		{
			A0 = A_ref_MEO + eph->dA;
		...
		a =  A0 + eph->Adot * tk;
		M  = eph->M0 + (sqrt(mu / A0 / A0 / A0)+eph->deln+0.5*eph->ndot*tk) * tk;
    ...
    // printf("xn:%20.10lf \n", (sqrt(mu / (eph->A * eph->A * eph->A)) + eph->deln));
    ...
    // printf("v:%lf omg:%lf\n", atan2(sqrt(1.0 - eph->e * eph->e) * sinE, cosE - eph->e), eph->omg);

    double v0 = 1.0 - eph->e * cosE;
    double vs = sqrt(1.0 - eph->e * eph->e) * sinE / v0;
    double vc = (cosE - eph->e) / v0;
    double v = fabs(asin(vs));
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    for (i = 0; i < nav->n; i++)
    {
        ...
        if (sys == SYS_GAL)
        {
            setseleph(sys, 2); /// using F-NAV ephemeris(E1/E5a) only, by zgj
            ...
            /*if (sel==1&&!(nav->eph[i].code&(1<<8))) continue; */  /* F/NAV */
                                                                    ...
                                                                    // continue; /* I/NAV */
                                                                    /// excluding I-NAV ephemeris, by zgj
                                                                    // if (sel == 2 && !(nav->eph[i].code & (1 << 1)))
            if (sel == 2 && !(nav->eph[i].code & (1 << 8)))         /* only pass F/NAV */
                continue;
...
static void s_output_eph(const eph_t *eph)
{
    double ep[6];
    char prn[256] = {0};
    satno2id(eph->sat, prn);
    time2epoch(eph->toc, ep);
    printf("%3s %04d %02d %02d %02d %02d %02d%19.12e%19.12e%19.12e\r\n", prn, (int)ep[0], (int)ep[1], (int)ep[2], (int)ep[3], (int)ep[4], (int)ep[5], eph->f0, eph->f1, eph->f2);
                      ...
                      int opt, double *rs, double *dts, double *var, int *svh)
{
    ...
    if (sys == SYS_GPS || sys == SYS_GAL || sys == SYS_QZS || sys == SYS_CMP)
    {
        ...
        // by zzy align to bamboo
        if (sys == SYS_CMP)
        {
            double fq0, fq1, a1, tgd;
            fq0 = code2freq(SYS_CMP, CODE_L2I, 0);
            fq1 = code2freq(SYS_CMP, CODE_L6I, 0);
            a1 = fq0 * fq0 / (fq0 * fq0 - fq1 * fq1);
            tgd = eph->tgd[0];
            *dts = *dts - a1 * tgd;
        }
        // by zzy
