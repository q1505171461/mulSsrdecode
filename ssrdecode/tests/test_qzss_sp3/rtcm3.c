...
const char *msm_sig_sbs[32] = {
    ...
    "", "", "", "", "", "", "", "", "", "5I", "5Q", "5X",
    "", "", "", "", "", "", "", ""};
const char *msm_sig_cmp[32] = { // by zzy
...
static int decode_type1042(rtcm_t *rtcm)
{
    ...
    // eph.toes = time2gpst(bdt2gpst(bdt2time(week,eph.toes)),&eph.week);
    // by zzy
    // if (rtcm->time.time == 0)
    //     rtcm->time = utc2gpst(timeget());
    ...
    eph.A = sqrtA * sqrtA;
    // by zzy
    // eph.iode = eph.iodc = ((int)eph.toes / 720) % 240; // IGS
    // eph.iode = eph.iodc = (int)(((double)fmod(eph.toes, 86400.0)) / 1800.0 + 0.5 * (((double)(fmod(eph.toes, 86400.0)) / 1800.0) > 0 ? 1 : -1)) + 1; // unknown
    // printf("%d,%lf\n", fmod(eph.toes, 86400.0), fmod(eph.toes, 86400.0));
    eph.iode = (int)(fmod(eph.toes, 86400.0) / 1800.0 + 0.5) + 1;
    if (!strstr(rtcm->opt, "-EPHALL"))
    {
...
static int decode_ssr4(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    for (j = 0; j < nsat && i + 191 + np + ni + nj <= rtcm->len * 8; j++)
    {
        ...
        rtcm->ssr[sat - 1].iode = sys == SYS_CMP ? iodcrc : iode; // by zzy
...
/* decode SSR 9 message header -----------------------------------------*/
// static int decode_ssr9_head(rtcm_t *rtcm, int sys, int subtype, int *sync,
//                             int *iod, double *udint, int *hsize,double *lat, double *lon, double *hig)
static int decode_ssr9_head(rtcm_t *rtcm, int sys, int subtype, int *sync,
                            int *iod, double *udint, int *hsize)
{
    char *msg, tstr[64];
    ...
    i = decode_ssr_epoch(rtcm, sys, subtype); // obtain time
    ...
    *sync = getbitu(rtcm->buff, i, 1); // SSR Multiple Message Number
    ...
    i += 3;
    rtcm->atmo.geod[0] = getbitu(rtcm->buff, i, 38) / 1e7 * (PI / 180); /*Reference latitude */
    i += 38;
    rtcm->atmo.geod[1] = getbitu(rtcm->buff, i, 39) / 1e7 * (PI / 180); /* Reference longitude */
    i += 39;
    rtcm->atmo.geod[2] = getbitu(rtcm->buff, i, 23) / 100.0; /* Reference height */
    i += 23;
...
/* decode SSR 9: STEC ---------------------------------------------------------*/
static int decode_ssr9(rtcm_t *rtcm, int sys, int subtype)
{
    float stec, sigma;
    ...
    if ((nsat = decode_ssr9_head(rtcm, sys, subtype, &sync, &iod, &udint, &i)) < 0)
    {
        trace(2, "rtcm3 %d length error: len=%d\n", type, rtcm->len);
        return -1;
    }
    ...
    for (j = 0; j < nsat && i + 48 <= rtcm->len * 8; j++)
    {
        prn = getbitu(rtcm->buff, i, np) + offp;
        i += np;
        elev = getbitu(rtcm->buff, i, 10) * 0.1 * (PI / 180);
        i += 10;
        stec = getbits(rtcm->buff, i, 14) * 0.05;
        i += 14;
        sigma = getbitu(rtcm->buff, i, 8) * 0.05;
        i += 8;
        i += 10;
        if (!(sat = satno(sys, prn)))
        {
        ...
        rtcm->atmo.time = rtcm->time;
        rtcm->atmo.elev[sat - 1] = elev;
        rtcm->atmo.ionval[sat - 1] = stec;
        rtcm->atmo.ionsig[sat - 1] = sigma;
    }
    return sync ? 0 : 9;
}
...
/* decode type 4065: proprietary message KPL add by xtl---------------------------------*/
static int decode_type4065(rtcm_t *rtcm)
{
    int i = 24 + 12, ver, subtype;
    //
    if (i + 4 + 8 >= rtcm->len * 8)
    {
        trace(2, "rtcm3 4065: length error len=%d\n", rtcm->len);
        return -1;
    }
    // no ver information in rtcm
    ver = getbitu(rtcm->buff, i, 3); // 0xF
    i += 3;
    // KPL Message Number length is 8
    subtype = getbitu(rtcm->buff, i, 8);
    i += 8;
    if (rtcm->outtype)
    {
    ...
    {
    case 101:
        return decode_ssr9(rtcm, SYS_GPS, subtype);
    case 102:
        return decode_ssr9(rtcm, SYS_CMP, subtype);
    case 103:
        return decode_ssr9(rtcm, SYS_GAL, subtype);
    case 104:
        return decode_ssr9(rtcm, SYS_GLO, subtype);
    case 105:
        return decode_ssr9(rtcm, SYS_QZS, subtype);
        // case 106:
        //     return decode_ssr9(rtcm, SYS_SBS, subtype);
    }
    trace(2, "rtcm3 4075: unsupported message subtype=%d\n", subtype);
    return 0;
}