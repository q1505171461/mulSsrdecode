 ...
 * @Date: 2022-09-12 23:07:27
 * @Last Modified by: zgj
 * @Last Modified time: 2022-09-16 17:56:34
 */
/*------------------------------------------------------------------------------
 * tinyppp.c : tinyppp common functions
 *
 *          Copyright (C) 2007-2020 by T.TAKASU, All rights reserved.
 *
 *          Revised by Juntao at wuhan university
 *
 *
 ...
 *                           rename api crc16()    -> crc16()
 *                           rename api crc24q()   -> crc24q()
 *                           rename api crc32()    -> crc32()
...
/* SELF DEFINED should change with the increased options */
...
const prcopt_t prcopt_default =
    {
        ...
        PMODE_PPP_KINEMA,                      /* PMODE */
        SPPOPT_IF,                             /* SPPOPT */
        10.0 * D2R,                            /* elmin */
        10.0 * D2R,                            /* elmin_a */
        {0, 240, 240},                         /* minlock */
        0.15,                                  /* slip threshold of eometry-free phase (m) */
        {{0, 0}},                              /* snrmask */
        0,                                     /* sateph */
        1,                                     /* ionopt */
        {5000, 9000, 0.2, 100, 100, 20},       /* initial std (pos,rclk,trop,l5dcb,p1p2dcb,iono) */
        {500, 9000, 0.0004, 0.002, 0.002, 20}, /* process noise (pos,rclk,trop,l5dcb,p1p2dcb,iono)*/
        {0, 0, 0},                             /* rover position */
        "ANTNONE",                             /* anttype */
        {0},                                   /* antdel */
        {0},                                   /* pcvr */
        "NONE",                                /* p1p2dcb opt */
        ...
        1,                                     /* yawmodel */
        "",                                    /* freqseq */
        {{"L1", "L2", "L5", ""}, {"L1", "L7", "L5", ""}, {"L2", "L6", "L5", ""}, {"L2", "L6", "L7", ""}, {"", "", "", ""}},
        /* freq */
        {3, 3, 3, 3, 0},                     /* n freq */
        {"L5", "L7", "L5", "L7"},            /* l5 freq */
        1,                                   /* interval */
        {0, 0},                              /* ts */
        {0, 0},                              /* te */
        3,                                   /* max frequency to process */
        {0.003, 0.003, 0.003, 0.003, 0.003}, /* phase observation noise */
        {0.3, 0.3, 0.3, 0.3, 0.3},           /* range obseration noise */
        {0},                                 /* ocean tide loading parameters */
        0,                                   /* real-time & post flag*/
        1,                                   /* dia configures*/
        1,                                   /* iar configures*/
        {0}                                  /* ambiguity configures*/
};
...
const int nprn_default = 96; /* global nprn, reading from configure file*/

const char cprn_default[MAXSAT][LEN_PRN] = {
    "G01",
    "G02",
    "G03",
    "G04",
...
extern int satno(int sys, int prn)
{
    ...
    {
    case SYS_GPS:
        sprintf(cprn, "G%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_GAL:
        sprintf(cprn, "E%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_CMP:
        sprintf(cprn, "C%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_QZS:
        sprintf(cprn, "J%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_GLO:
        sprintf(cprn, "R%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_IRN:
        sprintf(cprn, "I%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    case SYS_LEO:
        sprintf(cprn, "L%02d", prn);
        return pointer_string(nprn_, LEN_PRN, (char *)cprn_, cprn) + 1;
    }
    return 0;
}
...
extern int satsys(int sat, int *prn)
{
    ...
    if (prn)
        *prn = (sys == SYS_NONE) ? 0 : atoi(cprn_[sat - 1] + 1);
    return sys;
}
...
{
    char buff[LEN_PRN] = {0};
    strncpy(buff, id, LEN_PRN - 1);
    chop(buff);
    return pointer_string(nprn_, LEN_PRN, (char *)cprn_, buff) + 1;
}
 ...
 *-----------------------------------------------------------------------------*/
extern uint32_t crc32(const uint8_t *buff, int len)
{
    uint32_t crc = 0;
    int i, j;
 ...
 *-----------------------------------------------------------------------------*/
extern uint16_t crc16(const uint8_t *buff, int len)
{
    uint16_t crc = 0;
    int i;
...
extern void lubksb(const double *A, int n, const int *indx, double *b)
{
    double s;
    int i, ii = -1, ip, j;
...
extern void matfprintf_w(const Scalar A[], int tk, int n, int m, int p, int q, FILE *fp)
{
    ...
    {
        for (j = 0; j < m; j++)
            fprintf(fp, " %*.*f", p, q, A[i + j * tk]);
        fprintf(fp, "\n");
    }
    fflush(fp);
}

extern void matprintf_w(const Scalar A[], int tk, int n, int m, int p, int q)
{
    matfprintf_w(A, tk, n, m, p, q, stdout);
}
/* string to number ------------------------------------------------------------
...
extern int ymd2mjd(int iyear, int imonth, int iday)
{
    ...
    {
        trace(3, "ymd2mjd, incorrect argument %d %d %d\n", iyear, imonth, iday);
        exit(1);
    }
    iyr = iyear;
    if (imonth <= 2)
 ...
 * args: count and length of string array
 * return : index of string in string_array
 *-----------------------------------------------------------------------------*/
extern int pointer_string(int row, int col, const char *string_array, const char *string)
{
    int i;
...
static double iauAnp(double a)
{
    double D2PI = 6.283185307179586476925287; /* 2 * PI */
    double w;
    w = fmod(a, D2PI);
    if (w < 0)
        w += D2PI;
...
static double iauEra00(double dj1, double dj2)
{
...
extern void eci2ecef(gtime_t tutc, const double *erpv, double *U, double *dU, double *gmst)
{
    ...
    gtime_t tgps;
    double mjd_utc, era, dlmat[9] = {0}, WD[9] = {0};
    ...
    /* compute earth rotate angle */
    mjd_utc = time2mjd(tutc);
    era = iauEra00(mjd_utc + 2400000.5, 0);
    Rrot(era, OMGE, dlmat); /* in column major */
    ...
    matmul("NN", 3, 3, 3, 1.0, W, dlmat, 0.0, WD);
    matmul("NN", 3, 3, 3, 1.0, WD, NP, 0.0, dU_); /* U=W*Rz(gast)*N*P */
...
extern int sum(int *array, int ndim)
{
    int i, sum0 = 0;
    for (i = 0; i < ndim; i++)
        sum0 += array[i];
...
/* frequency by type */
extern double freqbytp(int isys, const char *obs, int fcn)
{
    switch (SYS[isys])
    {
    case 'G':
    ...
    case 'C':
        ...
        case '1':
            return FREQL1; /* B1C */
        case '2':
            return FREQ1_CMP; /* B1I */
        case '7':
            return FREQ2_CMP; /* B2I/B2b */
        case '5':
            return FREQL5; /* B2a */
        case '6':
            return FREQ3_CMP; /* B3 */
        case '8':
            return FREQE5ab; /* B2ab */
        }
        break;
    case 'R':
        switch (obs[1])
        {
        case '1':
            return FREQ1_GLO + DFRQ1_GLO * fcn; /* G1 */
        case '2':
            return FREQ2_GLO + DFRQ2_GLO * fcn; /* G2 */
        case '3':
            return FREQ3_GLO; /* G3 */
        case '4':
            return FREQ1a_GLO; /* G1a */
        case '6':
            return FREQ2a_GLO; /* G2a */
        }
        break;
    case 'E':
        switch (obs[1])
 ...
 *          double *e        O   cbuf-of-sight vector (ecef)
...
extern int index_sys(int sat)
{
    ...
    if (strlen(prn))
    {
        if (prn[0] != 'C')
        {
            return index_string(SYS, prn[0]);
        }
        else
        {
            /* special case for BDS */
            if (atoi(prn + 1) >= 19)
                return index_string(SYS, 'C');
            else
                return index_string(SYS, 'B');
        }
    }
    return -1;
}
...
int s_validobs(const obsd_t *obs, int iq)
{
    return (iq != -1 && *code2obs(obs->code[iq])) ? 1 : 0;
}
void s_checkobs(obsd_t *obs, int n)
{
    int i, iq, nq;
    for (i = 0; i < n; ++i)
    {
        for (iq = 0, nq = 0; iq < NFREQ; ++iq)
        {
            if (s_validobs(obs + i, iq))
            {
                if (obs[i].P[iq] == 0.0 || obs[i].L[iq] == 0.0)
                    obs[i].code[iq] = CODE_NONE;
            }
            if (s_validobs(obs + i, iq) && obs[i].P[iq] != 0.0 && obs[i].L[iq] != 0.0)
            {
                ++nq;
            }
        }
        if (nq < 2)
        { /* less than two frequency */
            for (iq = 0; iq < NFREQ; ++iq)
            {
                obs[i].code[iq] = CODE_NONE;
            }
        }
