...
#define NTRIP_AGENT "RTKLIB/" VER_TINYPPP
...
static int openfile_(file_t *file, gtime_t time, char *msg)
{
    ...
    { /* output/sync time-tag */
        ...
        {
            sprintf(tagh, "TIMETAG RTKLIB %s", VER_TINYPPP);
...
static int gentcp(tcp_t *tcp, int type, char *msg)
{
    ...
    // by zzy
    { /* client socket */
        if ((gethostbyname_r(tcp->saddr, &result, buffer, sizeof(buffer), &hp, &h_err)))
        {
...
static void send_srctbl(ntripc_t *ntripc, socket_t sock)
{
    ...
    p += sprintf(p, "Server: %s %s %s\r\n", "RTKLIB", VER_TINYPPP, VER_TINYPPP);
